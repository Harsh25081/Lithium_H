...
const registerUser =async function (req,res){
    ...
    return res.send({status:true,"Registered User Data is : ": datacreated})
}

const loginUser = async function(req,res){
      let body = req.body;
      let {email,password} = body
      let checkLoginCred = await userModel.findOne({"email":email,"password":password})
      if(!checkLoginCred){
        return res.send({status : false ,"msg":"Email id or Password is Incorrect"})
      }
      let token = jwt.sign(
        {
          userId: checkLoginCred._id.toString(),
          batch: "thorium",
...
const fetchUserData = async function(req,res){
      ...
      if(!user){return res.send({status:false,msg:"User Id is INVALID"})}
...
const updateData = async function(req,res){
      let userId = req.params.userId
      let body = req.body
      if(!userId){return res.send({status:false,msg:"Not a valid userId"})}
      let updateddata = await userModel.findOneAndUpdate({_id:userId},body,{new:true})
      return res.send({status:true,msg : updateddata})
}
